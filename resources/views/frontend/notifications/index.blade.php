        ...
        <div class="col-md-12">
            @can('notification_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.notifications.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $notification->id }}">
                                        ...
                                        <td>
                                            @can('notification_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.notifications.show', $notification->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('notification_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.notifications.edit', $notification->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('notification_delete')
                                                <form action="{{ route('frontend.notifications.destroy', $notification->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('notification_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.notifications.massDestroy') }}",
