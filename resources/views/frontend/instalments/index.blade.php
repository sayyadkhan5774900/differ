        ...
        <div class="col-md-12">
            @can('instalment_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.instalments.create') }}">
                            ...
                            <tbody>
                                @foreach($instalments as $key => $instalment)
                                    <tr data-entry-id="{{ $instalment->id }}">
                                        ...
                                        <td>
                                            @can('instalment_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.instalments.show', $instalment->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('instalment_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.instalments.edit', $instalment->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('instalment_delete')
                                                <form action="{{ route('frontend.instalments.destroy', $instalment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('instalment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.instalments.massDestroy') }}",
