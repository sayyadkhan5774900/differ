        ...
        <div class="col-md-12">
            @can('fee_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.fees.create') }}">
                            ...
                            <tbody>
                                @foreach($fees as $key => $fee)
                                    <tr data-entry-id="{{ $fee->id }}">
                                        ...
                                        <td>
                                            @can('fee_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.fees.show', $fee->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('fee_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.fees.edit', $fee->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('fee_delete')
                                                <form action="{{ route('frontend.fees.destroy', $fee->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('fee_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.fees.massDestroy') }}",
