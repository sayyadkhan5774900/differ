        ...
        <div class="col-md-12">
            @can('study_material_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.study-materials.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $studyMaterial->id }}">
                                        ...
                                        <td>
                                            @can('study_material_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.study-materials.show', $studyMaterial->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('study_material_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.study-materials.edit', $studyMaterial->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('study_material_delete')
                                                <form action="{{ route('frontend.study-materials.destroy', $studyMaterial->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('study_material_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.study-materials.massDestroy') }}",
