...
<div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
  <div class="carousel-inner">
    ...
    <div class="carousel-item @if($key==0) active @endif">
      <img class="d-block w-100" src="{{  $slider->background_iamge ? $slider->background_iamge->getUrl() : "" }}" alt="" height="580px">
      <div class="carousel-caption d-none d-md-block">
        <h5>{!! $slider->title !!}</h5>
        <p>{{$slider->body_text}}</p>
        @if($slider->button_link)
        <button class="btn btn-primary">{{$slider->button_name}}</button>
        @endif
      </div>
    </div>
    @endforeach
