        ...
        <div class="col-md-12">
            @can('batch_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.batches.create') }}">
                            ...
                            <tbody>
                                @foreach($batches as $key => $batch)
                                    <tr data-entry-id="{{ $batch->id }}">
                                        ...
                                        <td>
                                            @can('batch_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.batches.show', $batch->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('batch_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.batches.edit', $batch->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('batch_delete')
                                                <form action="{{ route('frontend.batches.destroy', $batch->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('batch_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.batches.massDestroy') }}",
